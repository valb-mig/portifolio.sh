...
export default function GitActivity({ lang, text }: GitActivityProps) {
  ...
  useEffect(() => {
    async function fetchGitHub() {
      try {
        const res    = await fetch("https://api.github.com/users/valb-mig/events/public?per_page=30");
        const events = await res.json();
        const pushEvents = events.filter((e: any) => e.type === "PushEvent");

        if (pushEvents.length > 0) {
          const latest = pushEvents[0];
          const commit = latest.payload.commits?.at(-1);
          setLastCommit({
            message: commit?.message ?? "sem mensagem",
            repo:    latest.repo.name.replace("valb-mig/", ""),
        ...
        const todayPushes = pushEvents.filter((e: any) => e.created_at.startsWith(today));
        const count      = todayPushes.reduce(
          (acc: number, e: any) => acc + (e.payload.commits?.length ?? 0), 0
        );
        setTodayCount(count);
      } catch {
        // silently fail
